...
var db = {
	new_message: (uqid, user_id, username, avatar, message, profile_link, socket_s, user_s) => {
		...
		return client.db.none("INSERT INTO chat_messages (uniq_id, username, user_avatar, created_time, message, profile_link, user_id, socket_sess, user_sess) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", [uqid, username, avatar, date, message, profile_link, user_id, socket_s, user_s]).catch((error) => {
			console.log(error); // logging
		});
	},
	message_by_uqid: (uqid) => {
		return client.db.one("SELECT * FROM chat_messages WHERE uniq_id = $1", uqid).then((result) => {
			return result;
		}).catch((error) => {
			console.log(error); // logging
		});
	...
	remove_message: (uqid) => {
		return client.db.result("DELETE FROM chat_messages WHERE uniq_id = $1", uqid).then((result) => {
	...
	get_last_messages: () => {
		return client.db.query('WITH sub AS (SELECT c.chat_id, c.uniq_id, c.username, c.user_avatar, c.created_time, c.message, c.profile_link, cp.user_id, cp.type, cp.active FROM chat_messages c LEFT JOIN chat_permissions cp ON c.user_id = cp.user_id ORDER BY c.chat_id DESC LIMIT 15) SELECT * FROM sub ORDER BY chat_id ASC').then((result) => {
			return result;
		}).catch((err) => {
	...
	chat_delete_messages: (user_id) => {
		...
		return client.db.query('SELECT * FROM chat_messages WHERE user_id = $1 AND created_time <= $2', [user_id, date_bef]).then((result) => {
			return Promise.map(result, (data) => {
				return data.uniq_id;
			}).then((uniq_id) => {
				return client.db.none("DELETE FROM chat_messages WHERE user_id = $1 AND created_time <= $2", [user_id, date_bef]).then(() => {
	...
	remove_action: (user_id, type) => {
		return client.db.none("DELETE FROM chat_mod_actions WHERE user_id = $1 AND type = $2", [user_id, type]).catch((error) => {
			console.log(error); // logging
		});
	},
	tob_exists: (user_id) => {
